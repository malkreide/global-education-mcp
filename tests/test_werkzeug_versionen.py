...
Sie stand an zweien: `ruff>=0.5.0` im `[dev]`-Extra und
`pip install ruff==0.16.1` als eigener Schritt in `ci.yml`. Der CI-Schritt lief
nach dem Install des Extras und gewann gegen pyproject — der Wert dort war
wirkungslos, und wer die Gates lokal fuhr, benutzte die jeweils neueste Version
statt der, gegen die die CI prueft. In dieser Umgebung war das 0.15.8 gegen
0.16.1.

`136ba34` hat das zusammengefuehrt: Der Pin steht jetzt in `pyproject.toml`,
der CI-Schritt ist weg. Festgehalten war das seither nur in der CLAUDE.md — und
derselbe Commit hat beim Umschreiben den Gate-Block darin zerstoert. Eine
Aussage in Prosa kann eben nicht nur veralten, sie kann auch kaputtgehen, ohne
dass etwas rot wird. Dieser Test haelt den Zustand stattdessen fest.

Der Rueckfall bleibt still: Er macht kein Gate rot, er laesst es lediglich mit
