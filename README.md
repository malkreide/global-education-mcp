...
| `oecd_list_education_datasets` | Education at a Glance Datensätze | OECD |
...
# Global Education Data MCP Server (English)

An MCP server for international education data comparison, developed by the City of Zurich School Department (Schulamt der Stadt Zürich).

**Data Sources:**
- **UNESCO UIS API**: 4,000+ indicators, all UNESCO member countries, no API key required
- **OECD SDMX API**: Education at a Glance, 38 OECD countries, no API key required

**Country codes:** ISO 3166-1 Alpha-3 (CHE=Switzerland, DEU=Germany, FIN=Finland, SGP=Singapore, etc.)

**Dual transport:** `stdio` for local Claude Desktop · `SSE` for cloud/remote deployment
